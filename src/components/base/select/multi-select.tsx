        ...
        <div className="relative flex w-full flex-1 flex-row flex-wrap items-center justify-start gap-1.5">
            {!isSelectionEmpty &&
                comboBoxContext?.selectedItems?.items?.map((value) => (
                    <span key={value.id} className="flex items-center rounded-md bg-white py-0.5 pr-1 pl-1.25 ring-1 ring-gray-300 ring-inset">
            ...
            <div className={cx("relative flex min-w-[20%] flex-1 flex-row items-center", !isSelectionEmpty && "ml-0.5", shortcut && "min-w-[30%]")}>
                <AriaInput
                    ...
                    className="w-full flex-[1_0_0] appearance-none bg-transparent text-md text-ellipsis text-gray-900 caret-alpha-black/90 outline-none placeholder:text-gray-500 focus:outline-hidden disabled:cursor-not-allowed disabled:text-gray-400 disabled:placeholder:text-gray-400"
                />
    ...
    return (
        <AriaGroup
            ...
            className={({ isFocusWithin, isDisabled }) =>
                cx(
                    "relative flex w-full items-center gap-2 rounded-lg bg-white shadow-xs ring-1 ring-gray-300 outline-hidden transition duration-100 ease-linear ring-inset",
